...
describe('GraphQL Cache', () => {
  ...
  describe('.watchItem()', () => {
    ...
    it('should clear the expire handler if exists', done => {
      const c = new Cache();
      const q = `{item}`;
      const vars = {aa: 10};
    ...
    describe('stopping it', () => {
      ...
      it('should remove the whole item if there is no more callbacks', done => {
        const c = new Cache({cacheExpirationTimeout: 50});
        const q = `{item}`;
        const vars = {aa: 10};
        ...
        expect(Object.keys(c.items).length).to.be.equal(1);
        expect(item.expireHandler).not.to.be.equal(undefined);
