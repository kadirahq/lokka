...
export class Lokka {
  ...
  createFragment(fragment) {
    ...
    // try to parse name
    const matchedName = fragment.match(/fragment\s+([A-Za-z0-9]+)\s+on\s+/);
    if (matchedName) {
      if (matchedName.length === 2) {
        const fragmentName = matchedName[1];
  ...
  _findFragments(queryOrFragment, fragmentsMap = {}) {
    ...
    if (matched) {
      ...
      fragmentNames.forEach(name => {
        ...
        // stop find if fragment name is already in fragmentsMap
        if ( fragmentsMap[name] === fragment) {
          return;
        }
